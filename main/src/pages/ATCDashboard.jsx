...
const ATCDashboard = () => {
  ...
  const fetchAircraft = useCallback(async () => {
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5001'}/api/atc/aircraft?${params}`);
  ...
  const fetchConflicts = useCallback(async () => {
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5001'}/api/atc/conflicts?${params}`);
      const data = await response.json();
